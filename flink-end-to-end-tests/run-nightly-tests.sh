...
# not working with jdk11 - disabling
...
# Not passing with both J9 and HotSpot - disabling for now
...
# fails with one error - disabling for now
...
# blinking test - disabling for now
# if [[ `uname -i` != 'aarch64' ]]; then
...
MVN_COMMON_OPTIONS="-fn -Dflink.forkCount=2 -Dflink.forkCountTestPackage=2 -Dfast -Pskip-webui-build"
